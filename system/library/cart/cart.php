	...
	public function getProducts() {
		...
		foreach ($cart_query->rows as $cart) {
			...
			if ($product_query->num_rows && ($cart['quantity'] > 0)) {
				...
				if( $cart['CrossSell'] == "Yes" )
					{
						$crosssaleamount = $this->db->query("select SalePrice from oc_crosssale_products where CrossSaleProduct=".$cart['product_id']);
...
//				echo $product_query->row['quantity'].":".$product_query->row['quantity']."<".$cart['quantity']; exit; 
	...
	public function add($product_id, $quantity = 1, $option = array(), $recurring_id = 0,$crosssale='',$parentProduct='') {
	
	if($crosssale!='')
	{
		
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "' AND parentProduct=$parentProduct");
	...
	{	
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "cart WHERE customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' AND product_id = '" . (int)$product_id . "' AND recurring_id = '" . (int)$recurring_id . "' AND `option` = '" . $this->db->escape(json_encode($option)) . "' AND CrossSell='No' AND parentProduct='' ");
	...
	public function remove($cart_id,$remove) {
		if($remove!='')
		{
			#echo "DELETE FROM " . DB_PREFIX . "cart WHERE product_id = '" . (int)$cart_id . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' and CrossSell='Yes' "; exit; 
			
			$exp = explode(".",$remove); 

			$this->db->query("DELETE FROM " . DB_PREFIX . "cart WHERE product_id = '" . (int)$cart_id . "' AND customer_id = '" . (int)$this->customer->getId() . "' AND session_id = '" . $this->db->escape($this->session->getId()) . "' and CrossSell='Yes' AND parentProduct=".end($exp));
		}
		else
		{
