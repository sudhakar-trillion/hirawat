<?php
class ControllerCatalogproducteditcrosssale extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('catalog/product_editcrosssale');
...
$data['breadcrumbs'][] = array(
	...
	//get the cross sale product 
	
	
	$CrossSaleId =  $_GET['CrossSaleId'];

	$details = $this->model_catalog_product->getCrossSaleProduct($CrossSaleId);
	
	if($details=='0')
		{
			
			$routeto = $this->url->link('catalog/product_viewcrosssale', '', true)."&token=".$this->session->data['token'];
			?>
            <script>
			window.location.href="<?PHP echo $routeto?>";
			</script>
            <?PHP
		}
		else
		...
		$data['CrosssaleID'] = $_GET['CrossSaleId'];
		
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('catalog/product_editcrosssale', $data));
