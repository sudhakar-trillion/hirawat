		...
		if( $this->hasChild($parent) ){
			...
			foreach( $data as $menu ){
				...
				if( $this->hasChild($menu['megamenu_id']) || $menu['type_submenu'] == 'html'){			
					...
					// commented by sudhakar to remove the hyper link for the menu which had submenu/s and added a new line of code
					//$output .= '<a class="dropdown-toggle" data-toggle="dropdown" href="'.$this->getLink( $menu ).'">';
					
					$output .= '<a class="dropdown-toggle" data-toggle="dropdown"  style="cursor:pointer">';
...
//			echo '<pre>' .print_r( $parent, 1 ); exit;
