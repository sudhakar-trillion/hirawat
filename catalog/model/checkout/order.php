	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				$qry = $this->db->query("select ocd.name, ocd.category_id from oc_product_to_category opc left join oc_category_description as ocd on ocd.category_id=opc.category_id where product_id=".(int)$product['product_id']);
				
				$catid = $qry->row['category_id'];
				$category = $qry->row['name'];
				
			/*  
				From line number 13 to 16 added by sudhakar, its is because to get the sales report based on category so by default
				there is no columns for the categor so iadded 
			*/	
				
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', category_id=".$catid.",category_name='".$category."'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['link'] = $order_info['store_url'] . 'order-info/' . $order_id;
				...
				if ($order_info['customer_id']) {
					...
					$text .= "<a href='".$this->config->get('config_url').'order-info/'.$order_id."'>View order</a>\n\n";
				...
				//if ($this->config->get('config_order_mail')) {
					...
					$mail->setTo('sudhakar@trillionit.com');
					$mail->setFrom($this->config->get('config_email'));
					$mail->setSender(html_entity_decode($order_info['store_name'], ENT_QUOTES, 'UTF-8'));
					$mail->setSubject(html_entity_decode($subject, ENT_QUOTES, 'UTF-8'));
					$mail->setHtml($this->load->view('mail/order', $data));
			...
			if ($order_info['order_status_id'] && $order_status_id && $notify) {
				...
				if ($order_info['customer_id']) {
					...
					$message .= "<a href='".$this->config->get('config_url').'order-info/'.$order_id."'>View order</a>\n\n";
				}
