...
class ControllerAccountReturn extends Controller {
	...
	public function index() {
		...
		foreach ($results as $result) {
			$data['returns'][] = array(
				...
				'href'       => $this->createLink('return-product-info/'.$result['return_id'])
			);
		}
		...
		$pagination->url = $this->createLink('return-products-list/{page}'); //$this->url->link('account/return', 'page={page}', true);
		...
		$data['continue'] = $this->createLink('my-account'); //$this->url->link('account/account', '', true);
	...
	public function info() {
		...
		$this->request->get['return_id'] = $crentpge[3];
		...
		if ($return_info) {
			...
			$data['continue'] = $this->createLink('return-products-list'); # $this->url->link('account/return', $url, true);
	...
	public function add() {
		...
		$this->request->get['order_id'] = $segments[3];
		$this->request->get['product_id'] = $segments[4];
		
	/*	echo $this->request->get['order_id']."<br>";
	...
	*/	
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->createLink('product-order-success'));
		}
	...
	$data['action'] = '';
		...
		$data['back'] = $this->createLink('my-account');
