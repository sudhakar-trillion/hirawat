...
/*
	...
	*/
	 var value = $('#search input[name=\'search\']').val();
		...
		if(value!='')
		{
			url = $('base').attr('href') + 'product-search/'+value;
		    location = url;		
		}
		else
			{
				$("#prd-search").modal('show');	
			}
...
$(document).ready(function()
	{
		/*
			$(".prdsearch").on('focus',function()
			{
				$(this).autocomplete({ source: 'index.php?route=requestdispatcher/requestdispatcher/getproducts' });	
			});
	*/
			...
			$(".prdsearch").on('keyup',function()
			{
				var term = $(this).val();
				$.ajax({
							url: 'index.php?route=requestdispatcher/requestdispatcher/getproducts' ,
							type: 'get',
							data:{'term':term},
							success:function(resp)
							{
								resp=JSON.parse(resp);
								$(".ui-autocomplete").css({'display':'block'}).html(resp);
							}
				});
      ...
      success: function(json) {
        ...
        if (json['success']) {
          ...
          $('#cart > ul').load('index.php?route=common/cart/info ul li');
		   ...
		   $('.cart-items').load('index.php?route=requestdispatcher/requestdispatcher/getcartitems');
        }
		else
		{
			// $('#notification').html('<div class="alert alert-success">' + json['success'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
    ...
    $.ajax({
      ...
      success: function(json) {
		...
		var href = window.location.href;
			...
			console.log(href.indexOf("my-cart"));
		if( href.indexOf("my-cart")>0)
		{
			location = href;
		}
